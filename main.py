...
# from UNet import Unet,resnet34_unet,Unet_64
...
def getArgs():
    ...
    parse.add_argument('--arch', '-a', metavar='ARCH', default='Dynamic_Convolution_attention_unet',
                       help='UNet/resnet34_unet/unet++/myChannelUnet/Attention_UNet/segnet/r2unet/fcn32s/fcn8s/Unet_64/SmaAt_UNet'
                            'NAM_attention_unet/Dynamic Convolution_attention_unet/DyC_NAM_attentionUNet'
                            'all_attention/up_Dynamic_Convolution_AttU_Net'          
                            'CA_UNet/CBAM_UNet/SimAM_UNet/Triplet_UNet'
                            'SE_AttU_Net'               
                            'DCD_AttU_Net')            #动态卷积
...
def getModel(args):
    ...
    if args.arch == 'Dynamic Convolution_attention_unet':
        from Dynamic_Convolution_attention_unet import Dynamic_Convolution_AttU_Net
        model = Dynamic_Convolution_AttU_Net().to(device)
        model = Triplet_UNet(3,1).to(device)

    if args.arch == 'DCD_AttU_Net':
        from DCD_attention_unet import DCD_AttU_Net
        model = DCD_AttU_Net(3, 3).to(device)
...
def val(model,best_iou,val_dataloaders):
    ...
    with torch.no_grad():
        ...
        hd_total = 0
        dice_total = 0
        sensitivity_total = 0                          #新加指标1
        precision_total = 0                            #新加指标2
        ...
        for x, _,pic,mask in val_dataloaders:
            ...
            sensitivity_total += get_sensitivity(mask[0],img_y)         #新加指标1
            precision_total  += get_precision(mask[0],img_y)            #新加指标2
        ...
        aver_sensitivity = sensitivity_total / num              #新加指标1
        aver_precision = precision_total / num                  #新加指标2

        #新加2个指标

        print('Miou=%f,aver_hd=%f,aver_dice=%f,aver_sensitivity=%f,aver_precision=%f' % (aver_iou,aver_hd,aver_dice,aver_sensitivity,aver_precision))

        # 新加2个指标
        logging.info('Miou=%f,aver_hd=%f,aver_dice=%f,aver_sensitivity=%f,aver_precision=%f' % (aver_iou,aver_hd,aver_dice,aver_sensitivity,aver_precision))
        ...
        return best_iou,aver_iou,aver_dice,aver_hd,aver_sensitivity,aver_precision

def train(model, criterion, optimizer, train_dataloader,val_dataloader, args):
    best_iou,aver_iou,aver_dice,aver_hd,aver_sensitivity,aver_precision = 0,0,0,0,0,0        #添加2评价指标
    ...
    dice_list = []
    hd_list = []
    sensitivity_list = []                           #添加两个指标
    precision_list = []                             #添加两个指标
    for epoch in range(num_epochs):
        ...
        best_iou, aver_iou, aver_dice, aver_hd,aver_sensitivity,aver_precision = val(model, best_iou, val_dataloader)    #添加两个评价指标
        iou_list.append(aver_iou)
        dice_list.append(aver_dice)
        hd_list.append(aver_hd)

        sensitivity_list.append(aver_sensitivity)             #添加两个指标
        precision_list.append(aver_precision)                 #添加两个指标
    ...
    metrics_plot(args, 'sensitivity&precision',sensitivity_list,precision_list)            #添加两个指标
                               #添加两个指标
...
def test(val_dataloaders,save_predict=False):
    ...
    with torch.no_grad():
        ...
        for pic,_,pic_path,mask_path in val_dataloaders:
            ...
            sensitivity = get_sensitivity(mask_path[0],predict)                #添加两个评价指标
            sensitivity_total += sensitivity

            precision = get_precision(mask_path[0],predict)                       #添加两个评价指标
        ...
        print('Miou=%f,aver_hd=%f,dv=%f,sensitivity=%f,precision=%f' % (miou_total / num, hd_total / num, dice_total / num, sensitivity_total / num,precision_total / num))


        #添加两个评价指标

        #logging.info('Miou=%f,aver_hd=%f,dv=%f' % (miou_total/num,hd_total/num,dice_total/num))
        logging.info('Miou=%f,aver_hd=%f,dv=%f,sensitivity=%f,precision=%f' % (miou_total / num, hd_total / num, dice_total / num, sensitivity_total / num, precision_total / num))
